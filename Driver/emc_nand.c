...
{
	uint16_t index;
	uint32_t TxBuffer [NAND_PAGE_SIZE/4];
 uint32_t RxBuffer [NAND_PAGE_SIZE/4];
...
//	count = ADDR_AREA;
	...
	{
		DeBug("RxBuffer[%d]=%d\r\n",index,RxBuffer[index] ,Infor_Infor);
	}
	for( WriteReadAddr.Page = 0; WriteReadAddr.Page < 2; WriteReadAddr.Page++ )
	{	
		...
		for (index = 0; index < NAND_PAGE_SIZE/4; index++ )
		{
			...
			DeBug("TxBuffer[%d]=%d\r\n",index,TxBuffer[index] ,Infor_Infor);
		}
